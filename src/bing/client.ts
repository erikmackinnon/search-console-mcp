import { fetch } from 'undici'; // Or use global fetch in Node 18+
...
export class BingClient {
    ...
    async getPageQueryStats(siteUrl: string, pageUrl: string): Promise<BingQueryStats[]> {
        return this.request<BingQueryStats[]>('GetPageQueryStats', { siteUrl, pageUrl });
    }

    async submitSitemap(siteUrl: string, sitemapUrl: string): Promise<void> {
        await this.request<void>('SubmitSitemap', { siteUrl, sitemapUrl });
    }

    async getSitemaps(siteUrl: string): Promise<any[]> {
        return this.request<any[]>('GetSitemaps', { siteUrl });
    }
